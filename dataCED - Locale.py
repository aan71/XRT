...
def main() -> None:
    ...
    try:
        # Get pending files
        pending_files = get_pending_files(config.local_pending_dir)
        
        if not pending_files:
            message = 'No files found in pending directory.'
            print(message)
            logger.info(message)
        else:
            ...
            for file_path in pending_files:
                ...
                try:
                    all_records_succeeded = processor.process_file(file_path)
                    
                    # Always delete the original file after processing
                    # (error records are saved separately in the error directory)
                    file_path.unlink()
                    
                    if all_records_succeeded:
                        print(f'File {file_path.name} processed successfully and deleted.')
                        logger.info(f'File {file_path.name} processed successfully and deleted.')
                    else:
                        print(f'File {file_path.name} processed with errors (see error directory). Original file deleted.')
                        logger.warning(f'File {file_path.name} processed with errors (see error directory). Original file deleted.')
                    
                except ValueError as validation_error:
                    # CSV validation errors
                    print(f'Validation error in file {file_path.name}: {validation_error}')
                    logger.error(f'Validation error in file {file_path.name}: {validation_error}')
                    continue
                    
                except pyodbc.Error as db_error:
                    # Database errors
                    sanitized_msg = sanitize_error_message(db_error)
                    print(f'Database error processing file {file_path.name}: {sanitized_msg}')
                    logger.error(f'Database error processing file {file_path.name}: {sanitized_msg}')
                    continue
                    
                except OSError as file_error:
                    # File system errors
                    print(f'File system error with {file_path.name}: {file_error}')
                    logger.error(f'File system error with {file_path.name}: {file_error}')
                    continue
        
        print('CED process has finished successfully!')
